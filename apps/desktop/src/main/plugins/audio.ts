...
const audio = () => {
  return {
    'vol-up': {
      label: 'Volume Up',
      fire: async (data) => {
        loudness.setVolume((await loudness.getVolume()) + (data.stepUp || 10))
      },
      inputs: [
        {
          type: 'number',
    ...
    'vol-down': {
      label: 'Volume Down',
      fire: async (data) => {
        loudness.setVolume((await loudness.getVolume()) - (data.stepDown || 10))
      },
      inputs: [
        {
          type: 'number',
    ...
    'vol-to': {
      label: 'Set Volume To',
      fire: async (data) => {
        loudness.setVolume(data.volume)
      },
      inputs: [
        {
          type: 'number',
    ...
    'mute-toggle': {
      label: 'Toggle Mute',
      fire: async (data) => {
        loudness.setMuted(!(await loudness.getMuted()))
      }
    }
  }
}
