...
export const saveAndLoad = () => {
  ipcMain.on('load-board', (event, useLastBoard) => {
    ...
    if (useLastBoard) {
      ...
      myBoard = dialog.showOpenDialogSync({
        properties: ['openFile'],
        filters: [{ name: 'DeckPad Board Configuration', extensions: ['board'] }]
      })
  ...
  const saveAsFunction = (event, data) => {
    let myBoard = dialog.showSaveDialogSync({
      // properties: ['openFile'],
      filters: [{ name: 'DeckPad Board Configuration', extensions: ['board'] }]
    })
    myBoard += !String(myBoard).endsWith('.board') ? '.board' : ''
